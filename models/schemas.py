...
# =========================================================
# Folder Ingestion Models
# =========================================================

class FolderValidationRequest(BaseModel):
    """Request to validate a folder path"""
    folder_path: str = Field(description="Path to the folder to validate")
...
class FolderValidationResponse(BaseModel):
    ...
    folder_path: str
    total_files: int = 0
    supported_files: List[FileInfoSchema] = Field(default_factory=list)
...
class FolderIngestionRequest(BaseModel):
    """Request to start folder ingestion"""
    folder_path: str = Field(description="Path to the folder to ingest")
...
class IngestionErrorSchema(BaseModel):
    ...
    file_path: str
    filename: str
    error_type: str = Field(description="Error type: read_error, processing_error, unsupported, duplicate")
    error_message: str
...
class FolderIngestionResponse(BaseModel):
    ...
    job_id: str
    status: str = Field(description="Job status: pending, validating, scanning, processing, completed, failed, cancelled")
    progress: IngestionProgressSchema
    errors: List[IngestionErrorSchema] = Field(default_factory=list)
    results: Optional[IngestionResultSchema] = None


class IngestionJobResponse(BaseModel):
    """Full ingestion job response"""
    job_id: str
    folder_path: str
    status: str
    progress: IngestionProgressSchema
